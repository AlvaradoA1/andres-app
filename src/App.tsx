...
function App() {
  const [messages, setMessages] = React.useState<{message: string, id : number}[]>([]);
...
const createMessage = async () => {
  ...
  await fetch(`http://localhost:3000/api/messages`, {
    method: 'POST', body: JSON.stringify(newMessage), headers: {
      "Content-Type": "application/json",
    },
  });
  fetchMessages();
...
const moveMessage = (dragIndex:any, hoverIndex:any) => {
  ...
  return (
    ...
    <div className="App" 
      ...
      <div>
        <Button variant="contained" style ={{marginBottom: '10px',    width: '80px',height: '30px',marginLeft: '4px',}} onClick = {deleteMessages}>Clear</Button>
        <Button variant="contained" style ={{marginBottom: '10px' , marginLeft: '4px',    width: '80px', height: '30px'}} onClick = {handleClick}>Submit </Button>
      </div>
